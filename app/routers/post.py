...
limit: int = 10, skip: int = 0, search:Optional[str] = ""):
    ...
    '''if user have to only get his own posts
...
def createposts(post: schemas.PostCreate, db: Session = Depends(get_db), current_user: int = 
...
def get_post(id: int,db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user)):
    ...
    # if just only show active user post
